...
import 'api_manager_util.dart';
export 'api_manager_util.dart';
...
class APIManager {
  ...
  Map<String, dynamic> headerJSON(bool isAuthRequired,
      ...
      if (GlobalVariable.userDetail.token != null) {
        if (GlobalVariable.userDetail.token!.token != null) {
          headers["Authorization"] =
              "Bearer ${GlobalVariable.userDetail.token!.token!}";
        }
      }
    }
    return headers;
  ...
  Future<dynamic> postRequest(
    ...
    if (await APIManagerUtils.checkConnection()) {
      await dio
        ...
        parsed = _handleError(context, error);
      });
    } else {
      //parsed = _handleError(context, DioError(type: DioErrorType.DEFAULT, requestOptions: null));
      if (CommonFunction.isDialogShowing == false && context.mounted) {
        CommonFunction.showAlert(
            context, appName, APIManagerConstants.networkError);
      }
      parsed = null;
    }
    return parsed;
  ...
  dynamic _handleError(BuildContext context, DioError error) {
      ...
      if (error.response!.statusCode == 401) {
        ...
        if (error.response!.data != null) {
          ...
          if (isRedirectTOLogin) {
            ...
            CommonFunction.showAlertWithCompletionBack(
                context, appName, message, onTapOk: () {
